...
> 실시간 주가 변동, 관련 뉴스, 시장 지표, 과거 수익률 계산을 한 흐름에서 조회할 수 있도록 Kafka 스트리밍과 Airflow 배치 ETL을 결합한 주식 데이터 프로젝트
...
- 한국 / 미국 시장 전환
- 시가총액 상위 종목 데이터 조회
- 환율, 금리, KOSPI, 뉴스심리지수 등 시장 지표 조회
- 과거 시점 기준 투자 수익률 계산

## 데이터 파이프라인

### Streaming
- `stock-prices` 토픽에서 한국/미국 시가총액 상위 종목의 실시간 주가를 수집
- 조건에 맞는 종목을 선별해 `target_to_news` 토픽으로 전달
- 관련 뉴스 수집 후 `news_to_s3` 토픽을 통해 중복을 제거하고 S3에 적재

### Batch ETL
- 국내 시총 Top100 수집 및 과거 주가 적재
- 미국 시총 Top100 수집 및 과거 주가 적재
- KOSPI, 환율, 금리, 뉴스심리지수 등 시장 지표 수집
- S3 원천 데이터를 Glue / Lambda / pandas 기반 전처리 후 Redshift 적재
